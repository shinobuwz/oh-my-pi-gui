/**
 * Read-only current-session status for the browser bridge.
 *
 * The status adapter deliberately reads only public extension context data. Session
 * usage is aggregated from the active branch (with a compatibility fallback to the
 * public all-entry reader), while Git is queried asynchronously with fixed argv. No
 * session content, provider configuration, credentials, costs, or TUI footer data
...
/**
 * Generation-scoped status adapter. Git refreshes are asynchronous and throttled;
 * all other values are read-only snapshots of the active context.
 */
