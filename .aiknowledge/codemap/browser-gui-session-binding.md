...
- 公开 SDK（当前路线）：`createAgentSession()`（`dist/core/sdk.d.ts`）→ `session.bindExtensions({ uiContext, mode })`（`dist/core/agent-session.d.ts` 的 `ExtensionBindings`）。当前 `src/host/*` 实现这条路线（实现提交 `e885180`）。
...
`src/core/request-store.js`（非批准语义与超时）、`test/host-session.test.js`、`test/host-ui-context.test.js`、`test/bridge-server.test.js`、git 历史提交 `0959a48` 的 change 文档（私有接缝路线的架构决策与残余，历史）。
